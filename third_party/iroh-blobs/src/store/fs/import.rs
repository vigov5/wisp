...
async fn import_path_impl(
    ...
    // A path with a registered handle is one the embedder can read but that
    // cannot be opened by name, so every question about it — does it exist,
    // how big is it, what is in it — has to go through the handle.
    let preopened_file = if super::preopened::is_registered(&path) {
        Some(super::preopened::open(&path)?)
    } else {
        None
    };
    if preopened_file.is_none() && !path.is_file() && !path.is_symlink() {
        return Err(io::Error::new(
            io::ErrorKind::InvalidInput,
    ...
    let size = match &preopened_file {
        Some(file) => file.metadata()?.len(),
        None => path.metadata()?.len(),
    };
    tx.send(AddProgressItem::Size(size)).await?;
    let import_source = if size <= options.inline.max_data_inlined {
        let data = match &preopened_file {
            Some(file) => {
                // A duplicate shares the file offset with the handle the
                // embedder registered, so read from the start explicitly
                // rather than from wherever it happens to be.
        ...
        // can't proceed.
        let file = match preopened_file {
            Some(file) => file,
            None => OpenOptions::new().read(true).open(&path)?,
        };
