...
/// Resolve the rendezvous base URL from an explicit override, an environment
/// value, or the compiled-in default, in that precedence.
///
/// The `env_url` is passed in rather than read here so this stays wasm-clean
/// (`std::env` is meaningless in a browser). `wisp-core` wraps this with a
/// native `resolve_server_url` that reads `WISP_RENDEZVOUS_URL`.
pub fn resolve_server_url_with_env(override_url: Option<&str>, env_url: Option<String>) -> String {
