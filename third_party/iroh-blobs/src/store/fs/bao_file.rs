            ...
            Some(EntryState::Complete {
                ...
                let data = match data_location {
                    ...
                    DataLocation::External(paths, size) => {
                        ...
                        // Not `File::open`: this entry is being reloaded from
                        // the db to serve it, and an external path may be one
                        // the embedder can read but nobody can reopen — see
                        // `super::preopened`.
                        let file = super::preopened::open(&path)?;
                        MemOrFile::File(FixedSize::new(file, size))
                    }
                };
